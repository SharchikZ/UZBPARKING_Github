...
public class RCC_Exhaust : RCC_Core {
    ...
    public float flameTime = 3f;        //  Flame time.
    ...
    private void Flame() {
        ...
        if (CarController.engineRunning) {
            ...
            if (((CarController.useExhaustFlame && CarController.engineRPM >= 3000 && CarController.engineRPM <= 5500 && CarController.throttleInput <= .25f && flameTime <= .5f) || CarController.boostInput >= .75f) || previewFlames) {

                flameTime += Time.deltaTime;
                subEmission.enabled = true;

                if (flameLight)
                    flameLight.intensity = flameSource.pitch * 1f * Random.Range(.9f, 1f);
