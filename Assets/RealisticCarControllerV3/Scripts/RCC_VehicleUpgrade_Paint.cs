...
public class RCC_VehicleUpgrade_Paint : MonoBehaviour {
    ...
    public void MetallicChanger(float value)
    {
        if (!bodyRenderer)
        {
        ...
        bodyRenderer.materials[index].SetFloat("_Smoothness", value); // 0.8 = высокая гладкость
        bodyRenderer.materials[index].SetFloat("_ClearCoatSmoothness", value);
        ModApplier.loadout.smoothness = value;
        ModApplier.SaveLoadout();
    }
