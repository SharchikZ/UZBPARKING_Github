...
public class RCC_WheelCollider : RCC_Core {
    ...
    [Space()]
    public float wheelWidth = .45f; //	Width of the wheel.
    [Range(-0.14f, .14f)] public float wheelOffset = -0.01f;     // Offset by X axis.
    ...
    // Обновляем WheelAlign(), чтобы радиус учитывался при позиционировании
    private void WheelAlign()
    {
        ...
        // Динамическое смещение вбок в зависимости от стороны колеса
        Vector3 offsetDirection = transform.right;
        float sideMultiplier = (transform.localPosition.x < 0f) ? -1f : 1f;
        wheelPosition += offsetDirection * (wheelOffset * sideMultiplier);
    ...
    public void SaveSuspensionData()
    {
        PlayerPrefs.SetFloat("UpensionDistance" + gameObject.name, WheelCollider.suspensionDistance); 
    }
    public void LoadSuspensionData()
    {
        if(PlayerPrefs.HasKey("UpensionDistance" + gameObject.name))
        {
            WheelCollider.suspensionDistance = PlayerPrefs.GetFloat("UpensionDistance" + gameObject.name, WheelCollider.suspensionDistance);
        }
    }
    public void SaveCamberData()
    {
        PlayerPrefs.SetFloat("UcamberDistance" + gameObject.name, camber);
    }
    public void LoadCamberData()
    {
        if (PlayerPrefs.HasKey("UcamberDistance" + gameObject.name))
        {
            camber = PlayerPrefs.GetFloat("UcamberDistance" + gameObject.name, camber);
        }
    }
    public void SaveOffsetData()
    {
        PlayerPrefs.SetFloat("UoffsetDistance" + gameObject.name, wheelOffset);
    }
    public void LoadOffsetData()
    {
        if (PlayerPrefs.HasKey("UoffsetDistance" + gameObject.name))
        {
            wheelOffset = PlayerPrefs.GetFloat("UoffsetDistance" + gameObject.name, wheelOffset);
        }
    }
