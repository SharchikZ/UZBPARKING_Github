...
public class RCC_CarControllerV3 : RCC_Core {
    ...
    private float currentWheelAngle = 0f;     // Текущий визуальный угол руля
    ...
    private void OtherVisuals()
    {
        ...
        // Сохраняем исходный поворот только один раз
        if (orgSteeringWheelRot.eulerAngles == Vector3.zero)
            orgSteeringWheelRot = SteeringWheel.transform.localRotation;

        // === Основная логика ===
        float targetAngle = steerInput * steerAngle * steeringWheelAngleMultiplier;

        // Ограничиваем максимальный угол поворота руля (очень рекомендуется)
        const float maxVisualAngle = 720f; // 2 полных оборота — нормально для большинства игр
        targetAngle = Mathf.Clamp(targetAngle, -maxVisualAngle, maxVisualAngle);

        // Плавное изменение угла
        currentWheelAngle = Mathf.Lerp(currentWheelAngle, targetAngle, Time.deltaTime * 12f);
        // Альтернатива (ещё более плавно):
        // currentWheelAngle = Mathf.SmoothDamp(currentWheelAngle, targetAngle, ref velocity, 0.08f);

        // Применяем поворот
        Quaternion targetRotation = orgSteeringWheelRot *
                                    Quaternion.AngleAxis(-currentWheelAngle, GetSteeringAxis());
    ...
    private Vector3 GetSteeringAxis()
    {
        ...
        {
            case SteeringWheelRotateAround.XAxis: return Vector3.right;
            case SteeringWheelRotateAround.YAxis: return Vector3.up;
